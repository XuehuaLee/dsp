...
static void BandPassFilter(Uint8* srcVector,int32* destVector,Uint8 num)
{
    ...
    for(count = 6 ; count < num ; count++)
        tempConvX[count] = ( (srcVector[count]>>adcshift )& 0x00FF) * B[0]  + ( (srcVector[count - 2 ]>>adcshift )& 0x00FF)* B[2] + ( (srcVector[count - 4]>>adcshift )& 0x00FF)  * B[4]+( (srcVector[count - 6]>>adcshift )& 0x00FF)  * B[6];
    //save  xInfo
...
static int TotalRightCount = 0;//位同步丢失
